...
int main(int argc, char *argv[])
{
    if (argc == 4){                                                                 //4 argumenty pre --log
        ...
        if (strcmp(argv[1],"--pow") != 0) {                                         //argv[1] sa musi rovnat --pow
            ...
            printf("  taylor_pow(%.5g,%.5g) = %.12g\n", y, x, taylor_pow(x,y,n));   //vypis exp. funkcie pomocou talyorovho polynomu
            printf("taylorcf_pow(%.5g,%.5g) = %.12g\n", y, x, taylorcf_pow(x,y,n)); //vypis exp. funkcie pomocou talyorovho polynomu
...
double mypow(double x, double y)                                                    //funkcia pre umocnovanie pomocou efektivnejsieho logaritmu
{
    ...
    cf_log = mylog(y);                                                              //priradenie log
    double my_pow = 1.0, new_ln = 1.0, new_x = 1.0;                                 // f(x) ^ 2 + my_pow pre prvu iteraciu 1+...
    ...
    while (rozdiel_pow > eps){
        ...
        new_x *= x;                                                                 //x, x^2, x^3...
        new_ln *= cf_log;                                                           //ln a,ln^2a (lna * lna)...
        my_pow += (new_x * new_ln)/fakt;                                            //vysledok podla vzorca
