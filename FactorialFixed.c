...
int main(int argc, char *argv[])
{
    ...
    else if (argc == 3){                                                            //3 argumenty pre MYLOG
        if (strcmp(argv[1],"--mylog") != 0) {                                       //argv[1] sa musi rvonat --mylog
            arg_kontrola();                                                         //stderr -> chybove hlasenie
        }else{
            char *znakX;                                                            //premenne na kontrolu vstupu
            double x = strtod(argv[2], &znakX);                                     //nacitanie x, strtod (string to double)
            if (*znakX != '\0'){
                arg_kontrola();
                return EXIT_FAILURE;
...
double taylor_pow(double x, double y, unsigned int n)                               //funkcia na vypocet exponenc pomocou taylor_polynom
{
    ...
    if (x < 0){                                                                     //log existuje len pre kladne cislo
        arg_kontrola();                                                             //chybove hlasnie
        return EXIT_FAILURE;                                                        //chybne ukonecne
    }
